...
class FilterRef():
	def __init__(self,sat,sensor,YYMMDD,bandNUM):
		...
		self.L1BasePath = '/RED1BDATA_A/SNR/XWW/file/%s/MYD021KM/%s/211/' % (self.sat, self.year)
				...
				if LonLatMask.any():
					...
					if SunSatMask.any():
						...
						if RefData2.any():
							...
							validRefPath = '/RED1BDATA_A/SNR/XWW/file/MODIS/resultChangeLoc/%s/Band%s/' % (self.YYMMDD, str(self.bandNum))
